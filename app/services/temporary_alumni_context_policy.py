...
ACTIVE-DATASET EXEMPTION:
stars-national, fsu-stars, and famu-stars are all shown to eligible
alumni UNCONDITIONALLY, even with zero active data (has_active_dataset
is still reported truthfully) - see
TEMPORARY_ALUMNI_SLUGS_EXEMPT_FROM_ACTIVE_DATASET_RULE below. An admin
can also see/select any of these immediately after seeding (to perform
the first import), via the separate legacy-fallback/generic-institution
rules in app.services.organization_context_service - this module only
ever affects alumni-role visibility/access.

Any FUTURE slug added to TEMPORARY_ALUMNI_COMPATIBLE_SLUGS without also
being added to TEMPORARY_ALUMNI_SLUGS_EXEMPT_FROM_ACTIVE_DATASET_RULE
would instead follow the ordinary generic rule (see
app.services.organization_context_service.build_available_contexts)
that an institution context with no active dataset is hidden from
non-admins - i.e. it would only become visible to alumni once it has an
active dataset. That is not the case for any of the three slugs above.
...
# Slugs shown to eligible alumni even with zero active data. Any slug NOT
# in this set still requires an active dataset before this compatibility
...
# why these dashboards can appear "empty" to alumni, which is acceptable
# only because it was explicitly reviewed/approved for each of them.
TEMPORARY_ALUMNI_SLUGS_EXEMPT_FROM_ACTIVE_DATASET_RULE = frozenset({"stars-national", "fsu-stars", "famu-stars"})
