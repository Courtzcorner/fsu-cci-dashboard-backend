...
# --------------------------------------------------------------------------
# Second additive batch of temporary accounts: Bellab, Owebva, Ellliew,
# JuliaS, CourtneyS. Created through the exact same
# `seed_temporary_accounts()` path/config as every other temporary
# account above - no new code, just five more entries in
# TEMPORARY_ACCOUNTS.
# --------------------------------------------------------------------------

SECOND_BATCH_ALUMNI_USERNAMES = ("Bellab", "Owebva", "Ellliew", "JuliaS")
SECOND_BATCH_ADMIN_USERNAME = "CourtneyS"
SECOND_BATCH_USERNAMES = SECOND_BATCH_ALUMNI_USERNAMES + (SECOND_BATCH_ADMIN_USERNAME,)


def test_bellab_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "Bellab").first()
    assert user is not None
    assert user.role == "alumni"


def test_owebva_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "Owebva").first()
    assert user is not None
    assert user.role == "alumni"


def test_ellliew_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "Ellliew").first()
    assert user is not None
    assert user.role == "alumni"


def test_julias_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "JuliaS").first()
    assert user is not None
    assert user.role == "alumni"


def test_courtneys_is_created_as_admin(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "CourtneyS").first()
    assert user is not None
    assert user.role == "admin"


def test_all_five_second_batch_accounts_can_log_in_with_testtest(client, db_session):
    _seed(db_session)
    for username in SECOND_BATCH_USERNAMES:
        response = _login_temp(client, username)
        assert response.status_code == 200, response.text


def test_all_five_second_batch_accounts_require_credential_change(client, db_session):
    _seed(db_session)
    for username in SECOND_BATCH_USERNAMES:
        response = _login_temp(client, username)
        body = response.json()
...
def test_second_batch_passwords_are_hashed_and_never_plaintext(client, db_session):
    _seed(db_session)
    for username in SECOND_BATCH_USERNAMES:
        user = db_session.query(User).filter(User.username == username).first()
        assert user.password_hash != "testtest"
...
def test_second_batch_accounts_have_correct_credential_state(client, db_session):
    _seed(db_session)
    for username in SECOND_BATCH_USERNAMES:
        user = db_session.query(User).filter(User.username == username).first()
        assert user.temporary_account_created_at is not None
...
def test_running_the_seed_twice_creates_no_duplicates_for_second_batch(client, db_session):
    _seed(db_session)
    _seed(db_session)
    for username in SECOND_BATCH_USERNAMES:
...
def test_second_batch_existing_account_is_left_untouched_and_reported(client, db_session):
    # Simulate "JuliaS" already existing (e.g. a manually created row) -
    # the seed must never overwrite its password or role, and must
    # report it rather than silently skipping.
    existing = User(username="JuliaS", password_hash=hash_password("SomeOtherPass1"), role="alumni")
    db_session.add(existing)
    db_session.commit()
    existing_password_hash = existing.password_hash

    results = dict(_seed(db_session))
    assert results["JuliaS"] == "already_completed"
...
def test_second_batch_does_not_affect_first_batch_or_permanent_users(client, admin_user, db_session):
    admin_password_hash_before = admin_user.password_hash
    admin_role_before = admin_user.role
...
# --------------------------------------------------------------------------
# Third additive batch of temporary accounts: melissar (admin), amelissa
...
# two more entries in TEMPORARY_ACCOUNTS.
# --------------------------------------------------------------------------

THIRD_BATCH_ADMIN_USERNAME = "melissar"
THIRD_BATCH_ALUMNI_USERNAME = "amelissa"
THIRD_BATCH_USERNAMES = (THIRD_BATCH_ADMIN_USERNAME, THIRD_BATCH_ALUMNI_USERNAME)


def test_melissar_is_created_as_admin(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "melissar").first()
    assert user is not None
    assert user.role == "admin"


def test_amelissa_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "amelissa").first()
    assert user is not None
    assert user.role == "alumni"


def test_both_third_batch_accounts_can_log_in_with_testtest(client, db_session):
    _seed(db_session)
    for username in THIRD_BATCH_USERNAMES:
        response = _login_temp(client, username)
        assert response.status_code == 200, response.text


def test_both_third_batch_accounts_require_credential_change(client, db_session):
    _seed(db_session)
    for username in THIRD_BATCH_USERNAMES:
        response = _login_temp(client, username)
        body = response.json()
...
def test_third_batch_passwords_are_hashed_and_never_plaintext(client, db_session):
    _seed(db_session)
    for username in THIRD_BATCH_USERNAMES:
        user = db_session.query(User).filter(User.username == username).first()
        assert user.password_hash != "testtest"
...
def test_third_batch_accounts_have_correct_credential_state(client, db_session):
    _seed(db_session)
    for username in THIRD_BATCH_USERNAMES:
        user = db_session.query(User).filter(User.username == username).first()
        assert user.must_change_credentials is True
...
def test_running_the_seed_twice_creates_no_duplicates_for_third_batch(client, db_session):
    _seed(db_session)
    _seed(db_session)
    for username in THIRD_BATCH_USERNAMES:
        matches = db_session.query(User).filter(User.username == username).all()
...
def test_third_batch_existing_account_is_left_untouched_and_reported(client, db_session):
    # Simulate "amelissa" already existing (e.g. a manually created row) -
    # the seed must never overwrite its password or role, and must
    # report it rather than silently skipping.
    existing = User(username="amelissa", password_hash=hash_password("SomeOtherPass1"), role="alumni")
    db_session.add(existing)
    db_session.commit()
    existing_password_hash = existing.password_hash

    results = dict(_seed(db_session))
    assert results["amelissa"] == "already_completed"
...
def test_third_batch_does_not_affect_earlier_batches_or_permanent_users(client, admin_user, db_session):
    admin_password_hash_before = admin_user.password_hash
    admin_role_before = admin_user.role
...
# --------------------------------------------------------------------------
# Fifth additive batch of temporary accounts: jordanl (alumni). Created
# through the exact same `seed_temporary_accounts()` path/config as every
# other temporary account above - no new code, just one more entry in
# TEMPORARY_ACCOUNTS.
# --------------------------------------------------------------------------


def test_jordanl_is_created_as_alumni(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "jordanl").first()
    assert user is not None
    assert user.role == "alumni"


def test_jordanl_can_log_in_with_testtest(client, db_session):
    _seed(db_session)
    response = _login_temp(client, "jordanl")
    assert response.status_code == 200, response.text


def test_jordanl_requires_credential_change(client, db_session):
    _seed(db_session)
    response = _login_temp(client, "jordanl")
    assert response.json()["user"]["must_change_credentials"] is True


def test_jordanl_password_is_hashed_and_never_plaintext(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "jordanl").first()
    assert user.password_hash != "testtest"
    assert user.password_hash.startswith("$2b$")
...
def test_jordanl_has_correct_credential_state(client, db_session):
    _seed(db_session)
    user = db_session.query(User).filter(User.username == "jordanl").first()
    assert user.must_change_credentials is True
    assert user.temporary_account_created_at is not None
...
def test_running_the_seed_twice_creates_no_duplicate_jordanl(client, db_session):
    _seed(db_session)
    _seed(db_session)
    matches = db_session.query(User).filter(User.username == "jordanl").all()
    assert len(matches) == 1


def test_jordanl_existing_account_is_left_untouched_and_reported(client, db_session):
    # Simulate "jordanl" already existing (e.g. a manually created row) -
    # the seed must never overwrite its password or role, and must
    # report it rather than silently skipping.
    existing = User(username="jordanl", password_hash=hash_password("SomeOtherPass1"), role="alumni")
    db_session.add(existing)
    db_session.commit()
    existing_password_hash = existing.password_hash

    results = dict(_seed(db_session))
    assert results["jordanl"] == "already_completed"
...
def test_fifth_batch_does_not_affect_earlier_batches_or_permanent_users(client, admin_user, db_session):
    admin_password_hash_before = admin_user.password_hash
    admin_role_before = admin_user.role
